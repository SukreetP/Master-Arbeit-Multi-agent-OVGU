...
import collections
import time
from prio_buf import PrioritizedReplayBuffer
...
com_obj.loadModel("C:\\Users\\Sukreet\\Thesis\\custom environment\\updated_implementation\\all experiments and trainings\\DQN prioritized replay buffer single part carry experiment\\single_part_test.spp".format(mod_path))
...
class Agent:
    def __init__(self,env):
        ...
        self.memory1 = PrioritizedReplayBuffer(self.replay_buffer_size)
        self.memory2 = PrioritizedReplayBuffer(self.replay_buffer_size)
        self.memory3 = PrioritizedReplayBuffer(self.replay_buffer_size)
        self.gamma = 0.999
        self.epsilon = 1.0
    ...
    def remember(self,a1_state,a2_state,a3_state,next_state_a1,next_state_a2,next_state_a3,action_a1,action_a2,action_a3,Reward_AGV1,Reward_AGV2,Reward_AGV3,done):
        ...
        self.memory1.add((a1_state, action_a1,next_state_a1, Reward_AGV1, done))
        self.memory2.add((a2_state, action_a2, next_state_a2, Reward_AGV2, done))
        self.memory3.add((a3_state, action_a3, next_state_a3, Reward_AGV3, done))
    ...
    def replay(self):
        if len(self.memory1.memory) and len(self.memory2.memory) and len(self.memory1.memory) < self.train_start:
            return

        transitions1, weights1, indices1 = self.memory1.sample(self.batch_size)
        transitions2, weights2, indices2 = self.memory2.sample(self.batch_size)
        transitions3, weights3, indices3 = self.memory3.sample(self.batch_size)

        a1_state, action_a1, states_next_a1, Reward_AGV1, dones = zip(*transitions1)
        a2_state, action_a2, states_next_a2, Reward_AGV2, dones = zip(*transitions2)
        a3_state, action_a3, states_next_a3, Reward_AGV3, dones = zip(*transitions3)
        ...
        td_errors1 = np.zeros(self.batch_size, dtype=np.float32)
        td_errors2 = np.zeros(self.batch_size, dtype=np.float32)
        td_errors3 = np.zeros(self.batch_size, dtype=np.float32)
        ...
        for i in range(self.batch_size):
            ...
            td_errors1[i] = abs(q_values_a1[i][a] - q_values_next_a1[i].max())
            td_errors2[i] = abs(q_values_a2[i][b] - q_values_next_a2[i].max())
            td_errors3[i] = abs(q_values_a3[i][c] - q_values_next_a3[i].max())

        self.memory1.update(indices1, td_errors1)
        self.memory2.update(indices2, td_errors2)
        self.memory3.update(indices3, td_errors3)
